...
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to query for existing Volume during CreateVolume", "volume", volName)
	...
	if len(volumes) == 1 {
		...
		klog.V(4).InfoS("Volume already exists in Availability Zone", "volumeID", volumes[0].ID, "size", volumes[0].Size, "zone", volumes[0].AvailabilityZone)
		return getCreateVolumeResponse(&volumes[0], ignoreVolumeAZ, req.GetAccessibilityRequirements()), nil
	} else if len(volumes) > 1 {
		klog.V(3).InfoS("Found multiple existing volumes with selected name during create", "volume", volName)
	...
	klog.V(4).InfoS("Backups enabled", "status", backupsAreEnabled)
	if err != nil {
	...
	if content != nil && content.GetSnapshot() != nil {
		...
		if err == nil && snap.Status != "available" {
			return nil, status.Errorf(codes.Unavailable, "Snapshot %s is not yet available. status: %s", snapshotID, snap.Status)
		}
		...
		if backupsAreEnabled && cpoerrors.IsNotFound(err) {
			...
			if err != nil {
				...
				return nil, status.Errorf(codes.NotFound, "Snapshot or Backup with ID %s not found", snapshotID)
			}
			if back.Status != "available" {
				// If the backup exists but is not yet available, fail.
				return nil, status.Errorf(codes.Unavailable, "Backup %s is not yet available. status: %s", snapshotID, back.Status)
			}
	...
	if err != nil {
		klog.ErrorS(err, "Failed to CreateVolume", "volume", volName)
	...
	klog.V(4).InfoS("CreateVolume() successfully created", "volumeID", vol.ID, "size", vol.Size, "zone", vol.AvailabilityZone, "cloud", volCloud)
...
func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.V(3).InfoS("Volume already deleted", "volumeID", volID, "cloud", volCloud)
	...
	klog.V(4).InfoS("DeleteVolume() successfully deleted volume", "volumeID", volID, "cloud", volCloud)
...
func (cs *controllerServer) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to AttachVolume")
	...
	if err != nil {
		klog.ErrorS(err, "Failed to WaitDiskAttached")
	...
	if err != nil {
		klog.ErrorS(err, "Failed to GetAttachmentDiskPath")
		return nil, status.Errorf(codes.Internal, "[ControllerPublishVolume] failed to get device path of attached volume: %v", err)
	}

	klog.V(4).InfoS("ControllerPublishVolume() successfully attached volume", "volumeID", volumeID, "instanceID", instanceID, "cloud", volCloud)
...
func (cs *controllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	...
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.V(3).InfoS("ControllerUnpublishVolume() assuming volume is detached, because node does not exist",
				"volumeID", volumeID, "instanceID", instanceID, "cloud", volCloud)
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
		return nil, status.Errorf(codes.Internal, "GetInstanceByID failed with error %v", err)
	}

	err = cloud.DetachVolume(instanceID, volumeID)
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.V(3).InfoS("ControllerUnpublishVolume assuming volume is detached, because it was deleted in the meanwhile",
				"volumeID", volumeID, "cloud", volCloud)
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
		klog.ErrorS(err, "Failed to DetachVolume")
		return nil, status.Errorf(codes.Internal, "Detach Volume failed with error %v", err)
	}
	...
	klog.V(4).InfoS("ControllerUnpublishVolume() successfully detached volume", "volumeID", volumeID, "instanceID", instanceID)
...
func (cs *controllerServer) ListVolumes(ctx context.Context, req *csi.ListVolumesRequest) (*csi.ListVolumesResponse, error) {
	...
	for idx := startIdx; idx < len(cloudsNames); idx++ {
		...
		klog.V(4).InfoS("ListVolumes retrieved entries", "entries", len(ventries), "cloud", cloudsNames[idx])
		...
		if maxEntries > 0 && len(cloudsVentries) == maxEntries {
			if nextPageToken == "" {
				if idx+1 == len(cloudsNames) {
					...
					klog.V(4).InfoS("ListVolumes completed with max entries", "entries", len(cloudsVentries))
			...
			klog.V(4).InfoS("ListVolumes() completed with max entries", "entries", len(cloudsVentries), "nextToken", string(data))
			return &csi.ListVolumesResponse{
				Entries:   cloudsVentries,
	...
	klog.V(4).InfoS("ListVolumes() completed with all entries", "entries", len(cloudsVentries))
	return &csi.ListVolumesResponse{
		Entries:   cloudsVentries,
		NextToken: "",
	}, nil
...
func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if name == "" {
		return nil, status.Error(codes.InvalidArgument, "[CreateSnapshot] Snapshot name must be provided in CreateSnapshot request")
	}

	if volumeID == "" {
		return nil, status.Error(codes.InvalidArgument, "[CreateSnapshot] VolumeID must be provided in CreateSnapshot request")
	}
	...
	if snapshotType == "backup" {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to query for existing Backup during CreateSnapshot")
		...
		if len(backups) == 1 {
			...
			if err != nil {
				klog.ErrorS(err, "Failed to get backup by ID", "backupID", backup.ID)
		...
		if item, ok := (req.Parameters)[openstack.BackupMaxDurationPerGB]; ok {
			...
			if err != nil {
				klog.ErrorS(err, "Setting backup-max-duration-seconds-per-gb failed due to a parsing error")
	...
	if !backupAlreadyExists {
		...
		if err = ctime.CheckValid(); err != nil {
			klog.ErrorS(err, "Error to convert time to timestamp")
		}

		snap.Status, err = cloud.WaitSnapshotReady(snap.ID)
		if err != nil {
			klog.ErrorS(err, "Failed to WaitSnapshotReady", "snapshotID", snap.ID)
	...
	if snapshotType == "snapshot" {
		klog.V(4).InfoS("CreateSnapshot() snapshot created", "name", name, "volumeID", volumeID)
	...
	if snapshotType == "backup" {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to WaitBackupReady")
			return nil, status.Error(codes.Internal, fmt.Sprintf("[CreateSnapshot] WaitBackupReady failed with error %v. Current backups status: %s", err, backup.Status))
		}
		...
		if err != nil {
			...
			return nil, status.Error(codes.Internal, fmt.Sprintf("[CreateSnapshot] GetBackupByID failed with error %v", err))
		}

		err = cloud.DeleteSnapshot(backup.SnapshotID)
		if err != nil && !cpoerrors.IsNotFound(err) {
			klog.ErrorS(err, "Failed to DeleteSnapshot")
			return nil, status.Error(codes.Internal, fmt.Sprintf("[CreateSnapshot] DeleteSnapshot failed with error %v", err))
		}
	}

	klog.V(4).InfoS("CreateSnapshot() backup from the snapshot created", "name", name, "volumeID", volumeID)
	return &csi.CreateSnapshotResponse{
		Snapshot: &csi.Snapshot{
...
func (cs *controllerServer) createSnapshot(cloud openstack.IOpenStack, name string, volumeID string, parameters map[string]string) (snap *snapshots.Snapshot, err error) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to query for existing Snapshot during CreateSnapshot")
	...
	if len(snapshots) > 1 {
		klog.InfoS("found multiple existing snapshots with selected name during create", "name", name)
	...
	if len(snapshots) == 1 {
		...
		klog.V(3).InfoS("Found existing snapshot from volume with ID", "name", name, "volumeID", volumeID)
		return snap, nil
	}
	...
	if err != nil {
		klog.ErrorS(err, "Failed to Create snapshot")
...
func (cs *controllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	...
	if err == nil && back != nil {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to delete backup", "cloud", volCloud)
	...
	if err != nil {
		if cpoerrors.IsNotFound(err) {
			klog.V(3).InfoS("DeleteSnapshot() snapshot already deleted", "snapshotID", id, "cloud", volCloud)
			return &csi.DeleteSnapshotResponse{}, nil
		}
		klog.ErrorS(err, "Failed to Delete snapshot", "snapshotID", id, "cloud", volCloud)
...
func (cs *controllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	if len(snapshotID) != 0 {
		...
		if err != nil {
			if cpoerrors.IsNotFound(err) {
				klog.V(3).InfoS("ListSnapshots() snapshot not found", "snapshotID", snapshotID, "cloud", volCloud)
	...
	if err != nil {
		klog.ErrorS(err, "Failed to ListSnapshots", "cloud", volCloud)
	...
	for _, v := range slist {
		...
		if err := ctime.CheckValid(); err != nil {
			klog.ErrorS(err, "Error to convert time to timestamp", "cloud", volCloud)
		}
...
func (cs *controllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	if volume.Size >= volSizeGB {
		...
		klog.V(2).InfoS("ControllerExpandVolume() volume has been already expanded", "volumeID", volumeID, "size", volume.Size, "cloud", volCloud)
	...
	if err != nil {
		klog.ErrorS(err, "Failed to WaitVolumeTargetStatus of volume", "volumeID", volumeID)
		return nil, status.Errorf(codes.Internal, "Volume %s not in target state after resize operation: %v", volumeID, err)
	}

	klog.V(4).InfoS("ControllerExpandVolume() resized volume", "volumeID", volumeID, "size", volSizeGB, "cloud", volCloud)
